...
// 定义LCD_SPI的DMA发送相关的宏
#define LCD_SPI_TX_DMA_RCU            RCU_DMA0
#define LCD_SPI_TX_DMA                DMA0
#define LCD_SPI_TX_DMA_CH             DMA_CH4
#define LCD_SPI_TX_DMA_CH_IRQ         DMA0_Channel4_IRQn
#define LCD_SPI_TX_DMA_CH_IRQ_HANDLER DMA0_Channel4_IRQHandler
...
void lcd_spi_send_bydma(uint8_t *data, uint16_t len)
{
    ...
    // 使能UART2发送DMA时钟
    rcu_periph_clock_enable(LCD_SPI_TX_DMA_RCU);

    // 初始化指定的DMA通道
    dma_deinit(LCD_SPI_TX_DMA, LCD_SPI_TX_DMA_CH);
    ...
    // 初始化DMA单次传输模式
    dma_single_data_mode_init(LCD_SPI_TX_DMA, LCD_SPI_TX_DMA_CH, &dma_init_struct);

    // 选择DMA通道的子外设
    dma_channel_subperipheral_select(LCD_SPI_TX_DMA, LCD_SPI_TX_DMA_CH, DMA_SUBPERI0);

    // 使能DMA通道
    dma_channel_enable(LCD_SPI_TX_DMA, LCD_SPI_TX_DMA_CH);

    // 使能DMA传输完成中断
    dma_interrupt_enable(LCD_SPI_TX_DMA, LCD_SPI_TX_DMA_CH, DMA_CHXCTL_FTFIE);

    // 使能DMA中断
    nvic_irq_enable(LCD_SPI_TX_DMA_CH_IRQ, 2, 2);

    spi_dma_enable(SPI1,SPI_DMA_TRANSMIT);
}

// SPI发送DMA中断处理函数
void LCD_SPI_TX_DMA_CH_IRQ_HANDLER(void)
{
    /* enter interrupt */
    rt_interrupt_enter();

    // 检查DMA传输完成标志
    if(dma_interrupt_flag_get(LCD_SPI_TX_DMA,LCD_SPI_TX_DMA_CH,DMA_INT_FLAG_FTF))
    {
        // 清除DMA传输完成标志
        dma_interrupt_flag_clear(LCD_SPI_TX_DMA,LCD_SPI_TX_DMA_CH,DMA_INT_FLAG_FTF);
        // 调用发送完成回调函数
