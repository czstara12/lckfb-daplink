...
- Produces: 保持 10 个页面缓冲、8 个块缓存、32 个文件对象、6 个目录对象和 20 行 LVGL 缓冲。

- [ ] **Step 1: 运行配置断言并确认失败**

使用 `rg` 检查目标宏值和 `PKG_ST_7789_HEIGHT*20`；预期因当前文件对象、目录对象和 LVGL 行数仍为 50、10 和 30 而失败。

- [ ] **Step 2: 修改最小配置**

保持 UFFS 页面缓冲为 10，仅修改块缓存、文件对象、目录对象和 LVGL 行数，不改变算法、接口或 DMA 属性。
...
执行干净重构建、ELF 分段检查、符号地址检查和 `git diff --check`。确认链接堆边界增量与取消的常驻线程动态分配有效载荷之和不少于 18 KiB、CCM 剩余 1～2 KiB、所有已知 DMA 缓冲仍位于 SRAM1。
