...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
//    extern void lcd_fill_array_spi(uint16_t x_start, uint16_t y_start, uint16_t x_end, uint16_t y_end, uint16_t *pcolor);
//    lcd_fill_array_spi(area->x1, area->y1, area->x2, area->y2, (uint16_t *)color_p);
#if 1
//  extern void LCD_Fill_AREA(uint16_t xsta, uint16_t ysta, uint16_t xend, uint16_t yend, uint16_t* color);
//
//  LCD_Fill_AREA(area->x1, area->y1, area->x2, area->y2, (uint16_t *)color_p);

    extern void LCD_Fill_AREA_WITH_DMA(uint16_t xsta, uint16_t ysta, uint16_t xend, uint16_t yend, uint16_t* color);

    LCD_Fill_AREA_WITH_DMA(area->x1, area->y1, area->x2, area->y2, (uint16_t *)color_p);
#else
    ...
    for(y = area->y1; y <= area->y2; y++) {
        for(x = area->x1; x <= area->x2; x++) {
            ...
            LCD_Fill(x, y, x+1,y+1,color_p->full);
            color_p++;
        }
    }
#endif
...
void lcd_spi_dma_callback(void)
{
    lv_disp_flush_ready(&disp_drv);
}
